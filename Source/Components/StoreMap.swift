...
struct StoreMap: View {
    ...
    var body: some View {
        Map(position: $position) {
            ForEach(appModel.storeAnnotations) { annotation in
                Annotation(annotation.store.name, coordinate: annotation.coordinate, anchor: .bottom) {
                    AlkoMarker()
                        .onTapGesture {
                            withAnimation {
                                appModel.selectedStore = annotation.store
                            }
                        }
                }
            }
        ...
        .onMapCameraChange { context in
            ...
            debounceTask = Task {
                ...
                await appModel.reloadAnnotations(mapSize: mapSize, region: context.region)
            }
        }
        .onChange(of: appModel.selectedStore) { _, newValue in
            if newValue == nil {
        ...
        .task {
            await appModel.initializeClusters()
            await appModel.reloadAnnotations(mapSize: mapSize, region: MKCoordinateRegion(
                center: CLLocationCoordinate2D(latitude: 60.1699, longitude: 24.9384),
                span: MKCoordinateSpan(latitudeDelta: 0.2, longitudeDelta: 0.2),
            ))
        }
